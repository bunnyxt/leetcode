...
向列表`l`中插入元素`x`到坐标`idx`：`l.insert(idx, x)`

移除列表`l`中坐标为`idx`的元素：`l.pop(idx)`，支持负坐标，或`del l[idx]`，支持切片`del l[start:stop]`

列表比大小：同元组，列表`l1 = (a1, b1, c1)`与`l2 = (a2, b2, c2)`比较大小，就是先比`a1 a2`之后`b1 b2`最后`c1 c2`；但相对元组而言，不推荐作为简单包装，因为占用内存要更大，且无法作为key，除非需要修改列表内的元素
...
寻找将`x`插入`l`，保持`l`有序的位置，最左边：`bisect.bisect_left(l, x)`，返回值同时也是`l`中`< x`的元素的个数
- 寻找并插入：`bisect.insort_left(l, x)`

寻找将`x`插入`l`，保持`l`有序的位置，最右边：`bisect.bisect_right(l, x)`，或者`bisect.bisect(l, x)`，返回值同时也是`l`中`<= x`的元素的个数
- 寻找并插入：`bisect.insort_right(l, x)`，或者`bisect.insort(l, x)`
